...
static void	close_other_fds(t_cmd *head, t_cmd *current)
{
	t_cmd	*c;
...
static void	pipe_child_io(t_cmd *cmd, t_cmd *head, int prev, int pw, int next_read)
{
	if (cmd->infile != -1)
	{
	...
	if (next_read != -1)
		close(next_read);
	close_other_fds(head, cmd);
}

static void	pipe_child(t_cmd *cmd, t_cmd *head, t_info *info, int prev, int pw, int nr)
{
	char	*path;
	char	**envp;
	...
	pipe_child_io(cmd, head, prev, pw, nr);
...
static int	pipe_run_one(t_cmd *cmd, t_cmd *head, t_info *info, int *prev, pid_t *pid)
{
	int	pipefd[2];
	...
	*pid = fork();
	...
	if (*pid == 0)
		pipe_child(cmd, head, info, *prev, pipefd[1], pipefd[0]);
	if (*prev != -1)
		close(*prev);
	if (cmd->infile != -1)
	{
...
static int	pipe_wait_all(pid_t *pids, int n)
{
	...
	while (i < n)
	{
		...
		if (pids[i] != -1 && WIFSIGNALED(status)
...
int	execute_pipeline(t_cmd *cmds, t_info *info)
{
	...
	while (cmd)
	{
		pipe_run_one(cmd, cmds, info, &prev, &pids[i++]);
		cmd = cmd->next;
	}
