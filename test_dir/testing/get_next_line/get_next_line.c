...
/*   Updated: 2025/10/08 22:54:55 by melshata         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*subfun(int fd, char *temp)
{
	...
	while (!schr(temp, '\n') && rs != 0)
	{
		...
		if (rs == -1)
			return (ffree(&buff, &temp, NULL));
		buff[rs] = '\0';
		tt = my_strjoin(temp, buff);
		if (!tt)
			return (ffree(&temp, &buff, NULL));
